...
const LightinigTable = () => {
    ...
    useEffect(() => {
        const fetchLight = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/Lightning/get-lightning-data')
                console.log(response.data.lightData)
                setLightData(response.data.lightData);
            } catch (error) {
    ...
    const deleteLight = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/Lightning/delete-lightning/${id}`);
        console.log(response.status);
        if (response.status === 200) {
         alert("Lightning Deleted");
          // Update the state to remove the deleted item
          setLightData(lightData.filter(item => item._id !== id));
        }
      } catch (error) {
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={lightData.lightImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
  ...
  const data = lightData.map((item, index) => ({
    key: index,
    _id: item._id,
    name: item.name,
    cost: item.cost,
