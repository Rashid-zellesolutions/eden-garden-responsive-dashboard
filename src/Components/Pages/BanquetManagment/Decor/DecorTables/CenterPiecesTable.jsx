...
const CenterTable = () => {
    ...
    useEffect(() => {
        const fetchCenter = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/CenterPieces/get-centerpiece-data')
                console.log(response.data.centerPieceData)
                setCenterData(response.data.centerPieceData);
            } catch (error) {
    ...
    const deleteCenter = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/CenterPieces/delete-centerpiece/${id}`);
        console.log(response.status);
        if (response.status === 200) {
         alert("Center Pieces Deleted");
          // Update the state to remove the deleted item
          setCenterData(centerData.filter(item => item._id !== id));
        }
      } catch (error) {
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={centerData.centerImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
