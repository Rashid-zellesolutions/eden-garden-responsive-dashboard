...
const BackdropTable = () => {
    ...
    useEffect(() => {
        const fetchBackdrop = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/BackdropAndMandap/get-backdrop-data')
                console.log(response.data.backdropData)
                setBackdropData(response.data.backdropData);
            } catch (error) {
    ...
    const deleteBackdrop = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/BackdropAndMandap/delete-backdrop/${id}`);
        console.log(response.status);
        if (response.status === 200) {
         alert("Backdrop Deleted");
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={backdropData.backdropImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
  ...
  const data = backdropData.map((item, index) => ({
    key: index,
    _id: item._id,
    name: item.name,
    cost: item.cost,
