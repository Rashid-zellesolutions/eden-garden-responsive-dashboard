...
const SeatingTable = () => {
    ...
    useEffect(() => {
        const fetchSeatingData = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/SeatingArrangments/get-seating-arrangmnts')
    ...
    const deleteSeating = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/SeatingArrangments/delete-seating-arrangments/${id}`);
        console.log(response.status);
        if (response.status === 200) {
         alert("Seating Deleted");
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={seatingData.seatingImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
