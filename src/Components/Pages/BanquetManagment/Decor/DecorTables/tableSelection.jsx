...
const TableSelection = () => {
    ...
    useEffect(() => {
        const fetchTableData = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/TableSelect/get-table-select')
                console.log(response.data.tableObj)
                setTableData(response.data.tableObj);
            } catch (error) {
    ...
    const deleteTable = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/TableSelect/delete-table-select/${id}`);
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={tableData.tableImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
