...
const Desserts = () => {
    ...
    useEffect(() => {
        const fetchDesserts = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/Dessert/get-dessert')
    ...
    useEffect(() => {
        const fetchDesserts = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/Dessert/get-dessert')
                console.log("Dessert Data", response.data.dessertObj)
                setDessertData(response.data.dessertObj);
            } catch (error) {
                console.error("Error Fetching Data", error);
            }
    ...
    const deleteDesserts = async (id) => {
        try {
            ...
            if (response.status === 200) {
                ...
                setDeleteData(dessertData.filter(item => item._id !== id));
            }
        } catch (error) {
    ...
    const handleDeleteService = async (id) => {
        try {
            await axios.delete(`http://localhost:8080/api/v1/Dessert/delete-dessert/${id}`);
    ...
    const columns = [
        {
            ...
            dataIndex: 'image',
            key: 'image',
            render: (img) => <img src={dessertData.dessertsImagePath} height={25} width={25} alt='img' />
        },
        {
            title: 'Name',
            dataIndex: 'name',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing && currentRecord) {
                const response = await axios.put(`http://localhost:8080/api/v1/Dessert/update-dessert/${currentRecord._id}`, formDataToSend, {
                    headers: {
                ...
                const response = await axios.post('http://localhost:8080/api/v1/Dessert/add-dessert', formDataToSend, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
