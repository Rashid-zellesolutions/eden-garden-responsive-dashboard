...
const TeaCoffe = () => {
    ...
    useEffect(() => {
        const fetchTeaCoffe = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/TeaCoffee/get-tea-coffee')
    ...
    const shouldShowPagination = teaCoffeData.length > 1;
    ...
    const deleteDesserts = async (id) => {
        try {
            ...
            if (response.status === 200) {
                ...
                setTeaCoffeData(teaCoffeData.filter(item => item._id !== id));
            }
        } catch (error) {
    ...
    const handleDeleteService = async (id) => {
        try {
            await axios.delete(`http://localhost:8080/api/v1/TeaCoffee/delete-tea-coffe/${id}`);
    ...
    const columns = [
        {
            ...
            dataIndex: 'image',
            key: 'image',
            render: (img) => <img src={teaCoffeData.teaCoffeImagePath} height={25} width={25} alt='img' />
        },
        {
            title: 'Name',
            dataIndex: 'name',
    ...
    const data = teaCoffeData.map((item, index) => ({
        key: index,
        _id: item._id,
        name: item.name,
        cost: item.cost,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing && currentRecord) {
                const response = await axios.put(`http://localhost:8080/api/v1/TeaCoffee/update-tea-coffe/${currentRecord._id}`, formDataToSend, {
                    headers: {
                ...
                setTeaCoffeData([...teaCoffeData, response.data]);
            } else {
                const response = await axios.post('http://localhost:8080/api/v1/TeaCoffee/add-tea-coffe', formDataToSend, {
                  headers: {
                    'Content-Type': 'multipart/form-data',
                },
                });
        ...
        <>
            <div style={{
                ...
                <div style={{ width: '100%', display: 'flex', padding: '0px 10px', justifyContent: 'start', marginTop: '10px', marginBottom: '10px' }}>
                    {packageVisible ? <h3>Add Tea Coffe</h3> : <Button type="primary" htmlType="submit"
                        style={{
