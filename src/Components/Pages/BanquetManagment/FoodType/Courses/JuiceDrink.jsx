...
const JuiceDrink = () => {
    ...
    useEffect(() => {
        const fetchJuices = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/JuicesDrinks/get-juices-drinks')
                console.log("Tea", response.data.juiceDrinkObj)
                setJuiceData(response.data.juiceDrinkObj);
    ...
    const handleDeleteService = async (id) => {
        try {
            await axios.delete(`http://localhost:8080/api/v1/JuicesDrinks/delete-juices-drinks/${id}`);
    ...
    const columns = [
        {
            ...
            dataIndex: 'image',
            key: 'image',
            render: (img) => <img src={juiceData.juiceDrinksImagePath} height={25} width={25} alt='img' />
        },
        {
            title: 'Name',
            dataIndex: 'name',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing && currentRecord) {
                const response = await axios.put(`http://localhost:8080/api/v1/JuicesDrinks/update-juices-drinks/${currentRecord._id}`, formDataToSend, {
                    headers: {
                ...
                const response = await axios.post('http://localhost:8080/api/v1/JuicesDrinks/add-juices-drinks', formDataToSend, {
                  headers: {
                    'Content-Type': 'multipart/form-data',
                },
                });
