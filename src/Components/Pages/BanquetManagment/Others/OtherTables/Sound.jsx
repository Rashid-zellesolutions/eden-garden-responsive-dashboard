...
const Sound = () => {
    ...
    useEffect(() => {
        const fetchSound = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/Sound/get-sound-data')
                console.log(response.data.soundObj)
                setSoundData(response.data.soundObj);
            } catch (error) {
    ...
    const deleteSound = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/Sound/delete-sound-data/${id}`);
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={soundData.soundImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
