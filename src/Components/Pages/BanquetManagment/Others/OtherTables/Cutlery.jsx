...
const Cutlery = () => {
    ...
    useEffect(() => {
        const fetchCutlery = async() => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/Cutlery/get-cutlery')
                console.log(response.data.cutleryObj)
                setCutleryData(response.data.cutleryObj);
            } catch (error) {
    ...
    const deleteCutlery = async (id) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/Cutlery/delete-cutlery/${id}`);
        console.log(response.status);
        if (response.status === 200) {
         alert("Cutlery Deleted");
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={cutleryData.cutleryImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
