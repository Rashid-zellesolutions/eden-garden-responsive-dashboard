...
const UpdateDesserts = () => {
    ...
    useEffect(() => {
        const fetchSingleProduct = async() => {
            try {
                const response = await axios.get(`http://localhost:8080/api/v1/Dessert/get-single-dessert/${id}`)
                console.log(response.data.dessertObj)
    ...
    const handleInputsChange = (e) => {
        ...
        console.log("Dessert Data after input change:", dessertData);
    }
    ...
    const handleImageChange = (e) => {
        setDessertData({...dessertData, dessertImage: e.target.files[0]});
    }
    
    console.log(dessertData.dessertImage);
    
    const handleDessertUpdate = async(e) => {
        e.preventDefault();
        // console.log(appetizerData.appetizerImage)
        const formData = new FormData();
        formData.append('name', dessertData.name);
        formData.append('cost', dessertData.cost);
        formData.append('pacFor', dessertData.pacFor);
        formData.append('dessertImage', dessertData.dessertImage);
        ...
        try {
            const response = await axios.put(`http://localhost:8080/api/v1/Dessert/update-dessert/${id}`, formData, {
                headers: {
  ...
  return (
    <div style={{width: '100%'}}>
        ...
        <div style={{width: '100%', padding: '10px 25px'}}>
            <form onSubmit={handleDessertUpdate} style={{width: '100%'}}>
                <div style={{
