...
const TeaCoffe = () => {
    ...
    const deleteTeaCoffe = async (id) => {
        try {
          const response = await axios.delete(`http://localhost:8080/api/v1/TeaCoffee/delete-tea-coffe/${id}`);
          console.log(response.status);
          if (response.status === 200) {
           alert("Tea Coffe Deleted");
      ...
      const handleMenuClick = (record, key) => {
        if (key === 'edit') {
          navigate(`/update-tea-coffe/${record._id}`, { state: { record } });
          console.log(record)
        } else if (key === 'delete') {
          deleteTeaCoffe(record._id);
        }
      };
    const menu = (record) => (
...
const columns = [
    {
      ...
      dataIndex: 'image',
      key: 'image',
      render: (img) => <img src={teaCoffe.teaCoffeImagePath} height={25} width={25} alt='img' />
    },
    {
      title: 'Name',
      dataIndex: 'name',
  ...
  const data = teaCoffe.map((item, index) => ({
    key: index,
    _id: item._id,
    name: item.name,
    cost: item.cost,
