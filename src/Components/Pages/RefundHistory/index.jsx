...
function RefundHistory() {
    const { GetAllRefund, refund, newBooking, loading } = useBookingContext()
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: 'refund',
            render: (record) => {
                const dateParts = record.date.split('-'); // Split the date string into parts
    ...
    const filteredData = refund?.length && refund?.map((item) => ({
        key: item.booking._id,
        booking: item.booking,
        refund: item.Refund,
    }))?.filter((record) => {
        const statusFilter = bookingStatusFilter ? record.booking.Status.toLowerCase().includes(bookingStatusFilter.toLowerCase()) : true;
        ...
        const customFilter =
            customStartDate && customEndDate
                ? new Date(record.refund.createdAt) >= customStartDate && new Date(record.refund.createdAt) <= customEndDate
                : true;
        ...
        const timeRangeFilter =
            range === "Last Week" || range === "This Month" || range === "Last Month" || range === "Last 3 Months" || range === "Last 1 Year"
                ? rangeStartDate(range, new Date(record.refund.createdAt))
                : true;
        return dateFilter && customerNameMatch && venueMatch && statusFilter && eventDateFilter && customerPhoneMatch && timeRangeFilter && customFilter;
    });
    ...
    return (
        <div style={{ padding: "20px" }}>
            {/* <h1 style={{ color: "#73787c", fontSize: "18px", marginBottom: "10px" }}>Refund Report</h1> */}
            {loading ? <Loader /> : <></>}
            <Collapse
