...
import InputField2 from "../../InputField/index2";
...
function SalesHistory() {
    ...
    const filteredData = payment?.length && payment?.map((item) => ({
        ...
        //     ? record.booking.venue.join(", ").toLowerCase().includes(venueFilter.toLowerCase())
        //     : true;
        // const dateFilter =
        //     startDate && endDate
        ...
        const dateFilter =
            startDate && endDate
                ? (
                    new Date(record.booking.createdAt) >= startDate &&
                    new Date(record.booking.createdAt) <= endDate
                ) || (
        ...
        const timeRangeFilter =
            range === "Last Week" || range === "This Month" || range === "Last Month" || range === "Last 3 Months" || range === "Last 1 Year"
                ? rangeStartDate(range, new Date(record.payment.createdAt))
                : true;
    ...
    return (
        <div style={{ padding: "20px" }}>
            {/* <h1 style={{ color: "#73787c", fontSize: "18px", marginBottom: "10px" }}>Sales Report</h1> */}
            {loading ? <Loader /> : <></>}
            <Collapse
