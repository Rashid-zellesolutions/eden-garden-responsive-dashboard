...
//   doc.text(`${mergeConsecutiveSlots(data.selectedSlot).join(", ")}`, 42, fromHeaderTop + 49);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text("SUB TOTAL", startX + 32, tableHeight + 11);

  doc.line(startX + 20, tableHeight + 15, 198, tableHeight + 15);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text(`$ ${data.summary.subTotal.toString()}`, startX + text1Width + 40, tableHeight + 11);

  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text("TIP", startX + 32, tableHeight + 20);
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'regular');
  doc.text(data.summary.tipType?.toString() === "%" ? `${data.summary.tip?.toString()} %` : `$ ${data.summary.tip?.toString()}`, startX + text1Width + 40, tableHeight + 20);


  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text(`TAX`, startX + 32, tableHeight + 28);
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'regular');
  doc.text(`${data.summary.tax} %`, startX + text1Width + 40, tableHeight + 28);


  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text("Discount", startX + 32, tableHeight + 36);
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'regular');
  doc.text(data.summary.discountType === "%" ? `${data.summary.discount?.toString()} %` : `$ ${data.summary.discount?.toString()}`, startX + text1Width + 40, tableHeight + 36);

  doc.line(startX + 20, tableHeight + 40, 198, tableHeight + 40);

  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text("Total", startX + 32, tableHeight + 46);
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'bold');
  doc.text(`$ ${data.summary.total.toString()}`, startX + text1Width + 40, tableHeight + 46);
  ...
  doc.rect(10, tableHeight + 7, startX - 10, 40);

  doc.setTextColor(0, 0, 0);
  doc.setFontSize(12);
  doc.text("PAYMENT TERMS:", 15, tableHeight + 16);


  doc.setTextColor(0, 0, 0);
  doc.setFontSize(12);
  doc.text("BANK TRANSFER:", 15, tableHeight + 32);

  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont("helvetica", 'regular');
  doc.text("BANK DETAILS", 15, tableHeight + 21);
  ...
  doc.addImage(stamp, 'JPEG', 70, tableHeight +11.5, 30, 30);
